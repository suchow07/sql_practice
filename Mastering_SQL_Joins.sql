...
-- 7.1: What is the average salary for the different gender?
 SELECT  e.gender, ROUND(AVG(s.salary)) as avg_salary
 from employees e 
...
-- employees database?
select e.gender, COUNT(*) as CountByGender
from employees e 
...
-- and department name
SELECT e.first_name, e.last_name, de.dept_no, e.hire_date, de.to_date, d.dept_name
FROM employees e
...
JOIN dept_emp de ON s.emp_no = de.emp_no 
JOIN departments d ON d.dept_NO = de.dept_no
WHERE ROUND(AVG(s.salary)) > 60000
GRoup by d.dept_name
ORDER BY avg_salary DESC;
